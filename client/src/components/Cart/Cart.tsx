...
export const Cart = () => {
  ...
  const cartItems = useSelector((state: RootState) => state.cartSlise.items);
  const allCategoryItems = useSelector((state: RootState) => state.itemsSlice.items);
  const allItems = []
        ...
        <div key={el.id} className="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
          ...
          <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
            <div className="mt-5 sm:mt-0">
              ...
              <p className="mt-1 text-xs text-gray-700">{title(el.category_id)}</p>
            </div>
